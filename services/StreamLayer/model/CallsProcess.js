const processData = (new_Call, calls_data) => {
    ...
    calls_data = processCallsPerGender(new_Call, calls_data);
    calls_data = processAverageWaitingTimePerHour(new_Call, calls_data);
    calls_data = processWaitingCalls(new_Call, calls_data);
    return calls_data;
};

const processCallsPerHour = (new_Call, calls_data) => {
    const hour = new Date(new_Call.start_time).getHours();
    calls_data.calls_per_hour.find((topic) => topic.name === new_Call.topic)
        .data[hour]++;
    return calls_data;
};
...
const processCallsPerGender = (new_Call, calls_data) => {
    if (!calls_data.calls_per_gender) {
        calls_data.calls_per_gender = {
            זכר: 0,
            נקבה: 0,
        };
    }
    calls_data.calls_per_gender[new_Call.gender]++;
    return calls_data;
};
...
const processAverageWaitingTimePerHour = (new_Call, calls_data) => {
    ...
    calls_data.average_waiting_time_per_hour[hour] = (
        calls_data.sum_of_waiting_time_per_hour[hour] /
        calls_data.total_calls_per_hour[hour]
    ).toFixed(2);
    calls_data.average_waiting_time_per_hour[hour];
    return calls_data;
...
const processWaitingCalls = (new_Call, calls_data) => {
    const waiting_calls = {
        x: new_Call.start_time,
        y: new_Call.waiting_calls,
    };

    if (calls_data.waiting_calls_array) {
        calls_data.waiting_calls_array.push(waiting_calls);
    } else {
        calls_data.waiting_calls_array = [waiting_calls];
    }
    if (calls_data.waiting_calls_array.length > 10) {
        calls_data.waiting_calls_array.shift();
    }
    return calls_data;
};
